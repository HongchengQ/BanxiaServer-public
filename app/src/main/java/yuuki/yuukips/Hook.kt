...
class Hook {
    // URL Server
    private var server = "http://bxgov.cn:21452"

    // App
    //private val package_apk = "com.miHoYo.Yuanshen"
    //private val package_apk = "com.xlpmy.dev"
    private val package_apk = "com.BanXia.Yuanshen"
    ...
    private fun Enter() {
        ...
        AlertDialog.Builder(activity).apply {
            setCancelable(false)
            setTitle("欢迎来到半夏公益服")
            setMessage("采用yuuki开源模块制作 不用创建账号 直接输账号密码即可成功登录\n第一次使用请直接直接点击[前往游戏]下载资源(默认进入热更新服务器)\n更新完资源重启游戏 点击[更改服务器]输入 1 切换为游玩服务器 以后再打开直接点击前往游戏即可\n如果想要回到热更新服务器 在[更改服务器]-处输入 0 即可\n同时[更改服务器]按钮直接输入其他服务器链接也能进入其他服务器游玩")
    ...
    private fun RenameJSON(){
        AlertDialog.Builder(activity).apply {
            setCancelable(false)
            setTitle("更改服务器")
            setMessage("[输入0切换至热更新地址]\n[输入1切换至游玩地址]\n[输入其他服务器地址则切换至该地址(如:http://示例1.com:666 或 https://示例2.com)]")
